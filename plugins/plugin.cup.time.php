<?php
//* plugin.cup.time.php - Cup Time
//* Version:   0.1



class plugin_cup_time extends FoxControlPlugin {
	...
	*/
	public function onStartUp() {
		$this->name = 'Cup Time';
		$this->author = 'Pastis-51,Momo';
		$this->version = '0.1';
		
		//Register Chat Command
		$this->registerCommand('cuptime', 'Displays ranking time', true);
	...
	*/
	public function showActiveList($login) {	
		...
		$window->title('$070C$fffup $070T$fffime');
		...
		for($i=0; isset($this->activeList[$currentID]) && $i<=24; $i++) {	
			...
			if($time != 0) {
				$window->content('<td width="3">'.($currentID+1).'</td><td width="19">'.htmlspecialchars($this->activeList[$currentID]['nickname']).'</td><td width="15">'.$this->activeList[$currentID]['login'].'</td><td width="15">$cf1'.$time.'</td><td width="15">'.$this->activeList[$currentID]['country'].'</td>');
			}
